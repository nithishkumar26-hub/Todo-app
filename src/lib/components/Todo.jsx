...
export const Todo = () => {
    ...
    function handleClick(){
        ...
        if(update && value ){
            ...
            if(todo.length == 0){
                ...
                console.log(value.trim().replace(/\s+/g, '').toLocaleUpperCase())
                console.log(todo.map((todo)=>todo.name.trim().replace(/\s+/g,'').toLocaleUpperCase()))
    ...
    function handleOkay(values){
        ...
        //     }
        // })
        setTodo(todo.map((todo)=>todo.id === values.id ? {...todo,id:todo.id,isCompleted:!todo.isCompleted} : todo))
        console.log(todo)
    }
    ...
    <div className='w-full min-h-screen   flex justify-center '>
        ...
        null}
            <div className=' mt-20 w-full '>
                ...
                <div className='w-full p-5 bg-[rgba(0,0,0,0.5)] text-white flex flex-col max-h-[600px] overflow-y-auto rounded '>
                    <span className='text-base text-red-600'>List of tasks</span>
                    {todo.length === 0 
                    ?
                    ...
                    todo.map((todos)=>(
                        ...
                        <ul className='flex justify-between my-2' key={todos.id}>
                            ...
                            break-words  ${todos.isCompleted ? "line-through text-slate-400 hover:bg-[rgba(0,0,0,0.8)]" : ""}`} onClick={()=>handleOkay(todos)}>{todos.name}</li>
